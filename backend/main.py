...
class MainWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        main_box.append(self.api_status)
    ...
    def check_api_status(self):
        ...
        is_healthy = loop.run_until_complete(self.check_api_health())
        if is_healthy:
            self.api_status.set_text("API Status: Connected")
            self.api_status.set_css_classes(["status-good"])
        else:
            self.api_status.set_text("API Status: Disconnected")
            self.api_status.set_css_classes(["status-bad"])

    async def process_image_with_api(self, image_path):
        try:
            # Prepare image for API request
            with open(image_path, 'rb') as f:
                files = {'file': f}
                async with aiohttp.ClientSession() as session:
                    async with session.post(f"{self.api_url}/detect_objects/", data=files) as response:
    ...
    def on_imagecapture_clicked(self, widget):
        ...
        def on_response(dialog, response_id):
            if response_id == Gtk.ResponseType.ACCEPT:
                ...
                # Get additional analysis from API
                loop = asyncio.get_event_loop()
                api_result = loop.run_until_complete(self.process_image_with_api(input_image_path))
                
                if api_result and 'caption' in api_result:
                    # Show caption in a dialog
                    caption_dialog = Gtk.MessageDialog(
                        transient_for=self,
                        modal=True,
                    ...
                    caption_dialog.format_secondary_text(api_result['caption'])
                    caption_dialog.show()
